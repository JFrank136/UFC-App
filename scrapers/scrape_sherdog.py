    ...
    try:
        ...
        def extract_total(section_name):
            try:
                ...
                print(f"[WARN] Error extracting {section_name}: {e}")
...
def main():
    ...
    if os.path.exists(OUTPUT_FILE):
        print("[TRASH] Starting fresh Sherdog scrape (existing data will be replaced)...")
    ...
    print(f"[START] Starting concurrent scraping with {max_workers} workers...")
    ...
    if failures:
        ...
        print(f"\n[WARN] Logged {len(failures)} failures to errors/sherdog_failures.json")
    ...
    print(f"[TARGET] Target fighters: {expected_count}")
    print(f"[OK] Successfully scraped: {final_count}")
    print(f"[X] Failed: {len(failures)}")
    print(f"[UP] Success rate: {(final_count/expected_count*100):.1f}%")
...
def run_retry_only():
    ...
    if not retry_these:
        print("[OK] No retryable failures.")
        return
    ...
    for entry in retry_these:
        ...
        if matched_fighter:
            ...
            print(f"[WARN] Fighter '{name}' not found in active roster, using error data")
    ...
    for i, fighter in enumerate(to_retry, 1):
        ...
        print(f"[SEARCH] Processing: '{name_raw}'")
        ...
        for ufc_name, sherdog_name in NAME_FIXES.items():
            if sherdog_name.upper() == name_raw.upper():
                ...
                found_reverse = True
                print(f"[RETRY] Reverse mapped: '{name_raw}' → '{search_name}'")
                break
        ...
        if not found_reverse:
            ...
            if search_name != original_search:
                print(f"[TOOL] Direct mapped: '{name_raw}' → '{search_name}'")
            else:
                print(f"[X] No NAME_FIXES mapping for: '{name_raw}'")
        
        # Check URL overrides BEFORE calling search_sherdog
        override_url = URL_OVERRIDES.get(search_name.upper())
        if override_url:
            print(f"[LINK] URL Override found: '{search_name}' → {override_url}")
            url = override_url
        else:
            print(f"[X] No URL Override for: '{search_name}' (checking: '{search_name.upper()}')")
            url = search_sherdog(search_name)
        
        if url:
            print(f"[OK] Final URL: {url}")
        else:
            print(f"[X] No URL found for: '{search_name}'")
        if not url:
    ...
    print(f"[STATS] Total fighters after merge: {len(merged)}")
    ...
    print(f"\n[TARGET] Retry complete. {len(results)} fighters saved. {len(still_failed)} still failed.")
