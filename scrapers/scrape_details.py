    ...
    try:
        ...
        if image_el and image_el.get("src"):
            ...
            if "ufc.com" not in image_url.lower():
                logger.warning(f" Non-UFC image URL for {fighter_slug}: {image_url}")
                image_url = None
        else:
            backup_selectors = [
                "img[src*='athlete_bio']",
            ...
            if not image_url:
                logger.warning(f" Could not find any image for {fighter_slug}")
        ...
        for fighter_rank in division.get("fighters", []):
            ...
            if rank == "C" or (isinstance(rank, int) and 1 <= rank <= 5):
                if uuid and uuid in uuid_to_fighter:
                    ...
                    logger.warning(f"   Fighter not found in roster: {name} (UUID: {uuid})")
    ...
    if mode == "1":
        ...
        for entry in failures:
            ...
            if fighter:
                ...
                print(f" Fighter '{name}' not found in active roster")
