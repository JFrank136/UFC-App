...
class SessionManager:
    """Browser-based session manager using undetected_chromedriver"""
    ...
    def _setup_driver(self):
        ...
        driver = launch_undetected_chrome(build_options)
    ...
    def get_page_source(self, url):
        ...
        try:
            ...
            print(f"    ERROR: Browser failed for {url}: {type(e).__name__}: {e}")
            self.record_error()
            return None
...
def search_tapology(fighter_name, debug=False):
    ...
    for attempt_num, search_name in enumerate(unique_attempts, 1):
        ...
        try:
            page_source = session_manager.get_page_source(url)
            if not page_source:
                if debug:
                    print(f"  Search failed: no page source returned")
                continue
